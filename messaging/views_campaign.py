...
def _user_id(view):
    """Return authenticated user's id, or None."""
    user = getattr(view.request, "user", None)
...
class CampaignListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        # Short-circuit during schema gen or anonymous requests
        if _is_schema_or_anon(self):
            return Campaign.objects.none()

        queryset = (
            Campaign.objects.filter(created_by_id=_user_id(self))
            .select_related("template")
            .prefetch_related("target_contacts", "target_segments")
        )
...
def start_campaign(request, campaign_id):
    ...
    try:
        campaign = get_object_or_404(
            Campaign, id=campaign_id, created_by=request.user
        )
        ...
        return Response(
            {"success": False, "message": "Failed to start campaign"},
...
def pause_campaign(request, campaign_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": "Failed to pause campaign"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def campaign_permissions(request, campaign_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": "Failed to get campaign permissions"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def campaign_analytics(request, campaign_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": "Failed to get campaign analytics"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def duplicate_campaign(request, campaign_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "message": "Failed to duplicate campaign"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def user_campaigns_summary(request):
    ...
    try:
        ...
        recent_campaigns = (
            Campaign.objects.filter(created_by=user).order_by("-created_at")[:5]
        )
        ...
        return Response(
            {"success": False, "message": "Failed to get campaign summary"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
