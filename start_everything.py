...
This script starts both servers and opens the purchase page.
"""
...
def find_purchase_page_port():
    """Find which port the purchase page is running on"""
    ports = [8080, 8081, 8082, 8083, 8084]
    for port in ports:
        try:
            response = requests.get(f"http://localhost:{port}/purchase_packages.html", timeout=2)
...
def start_purchase_page():
    ...
    port = find_purchase_page_port()
    if port:
        print(f"Purchase page server is already running on port {port}!")
        return f"http://localhost:{port}/purchase_packages.html"
    ...
    try:
        ...
        # Wait for purchase page to start
        print("Waiting for purchase page to start...")
        for i in range(10):
            time.sleep(1)
            port = find_purchase_page_port()
            if port:
                print(f"Purchase page server started on port {port}!")
                return f"http://localhost:{port}/purchase_packages.html"
            print(f"Waiting... ({i+1}/10)")
...
def main():
    print("Starting Mifumo SMS System")
    print("=" * 50)
    ...
    if not page_url:
        print("Failed to start purchase page server. Please run: python serve_purchase_page.py")
        return
